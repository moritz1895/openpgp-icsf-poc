...
/**
 * Erzeugt fuer die CLI-Demo echte JCA-Schluesselpaare und uebersetzt deren oeffentlichen Teil in
 * die von {@code PgpKeyMaterialCodec} (Bouncy-Castle-Bridge) dokumentierte Rohbyte-Kodierungskonvention.
 *
 * <p><b>Kein Produktivcode im eigentlichen Sinn:</b> diese PoC nimmt an, dass Schluessel bereits
 * vorab im HSM vorhanden sind (siehe Projektplan, "Keys are pre-provisioned"). Fuer die Demo
 * muessen solche Schluessel dennoch irgendwoher kommen - diese Klasse simuliert das, indem sie
 * frische Schluesselpaare erzeugt und ueber {@link InMemoryHsmKeyStore} als bereits im
 * (simulierten) HSM vorhanden registriert. Spiegelt bewusst denselben Ansatz wie die
 * gleichnamige Testhilfe {@code PgpTestKeys} in der Testquelle der Bouncy-Castle-Bridge, die
 * hier bewusst nicht direkt wiederverwendet wird - sie liegt im Testbaum und ist dort auf die
 * dortigen Testfaelle (u. a. P-384) zugeschnitten.</p>
 */
final class DemoKeyMaterial {
    ...
    static PgpPublicKey compositeMlKem768X25519PublicKey(KeyPair x25519KeyPair, KeyPair mlKem768KeyPair) {
        byte[] ecdhPublicKey = rawXdhPoint(x25519KeyPair);
        byte[] mlkemPublicKey = rawMlKemPublicKey(mlKem768KeyPair);
        return new PgpPublicKey(
                PgpPublicKeyAlgorithm.ML_KEM_768_X25519,
                ByteSequence.of(CompositeMlKemKeyMaterial.compose(ecdhPublicKey, mlkemPublicKey)));
    }
    ...
    private static int[] readDerTlvHeader(byte[] data, int[] cursor) {
        ...
        return new int[] {data[offset] & 0xFF, length};
    }
